...
//            std::cout<< std::endl <<"end printing node" << id << std::endl;
    ...
    class pr : public ParserExpression {
        public:        
        pr(ParserExpression* o) {name = "SENFLD"; op = o; expr = NULL; main = true; }
    };
...
%token NAME START REPLACE SENFLD PUNCHEDCARD
%token STAR_EXPR NUM ID

%type<str> STAR_EXPR NUM ID SENFLD PUNCHEDCARD
%type<oper> OPS TERM ARG LIST_OP OP
%type<expr> NAME START REPLACE 
%type<args> ARGS
...
|	START'='TERM		{ $$ = new ParserExpression("START", NULL , $3 , true); }
|	REPLACE'='TERM		{ $$ = new ParserExpression("REPLACE", NULL , $3 , true); }
;
