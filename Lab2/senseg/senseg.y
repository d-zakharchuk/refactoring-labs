...
//            std::cout<< std::endl <<"end printing node" << id << std::endl;
    ...
    class pr : public ParserExpression {
        public:        
        pr(ParserExpression* o) {name = "SENSEG"; op = o; expr = NULL; main = true; }
    };
...
%token NAME PARENT PROCOPT SSPTR INDICES SENSEG PUNCHEDCARD
%token STAR_EXPR NUM ID

%type<str> STAR_EXPR NUM ID SENSEG PUNCHEDCARD
%type<oper> OPS TERM ARG LIST_OP OP
%type<expr> NAME PARENT PROCOPT SSPTR INDICES 
%type<args> ARGS
...
|	INDICES'='TERM		{ $$ = new ParserExpression("INDICES", NULL , $3 , true); }
;
