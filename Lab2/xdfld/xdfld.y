...
//            std::cout<< std::endl <<"end printing node" << id << std::endl;
    ...
    class pr : public ParserExpression {
        public:
        pr(ParserExpression* o) {name = "XDFLD"; op = o; expr = NULL; main = true; }
    };
...
%token NAME SEGMENT CONST SRCH SUBSEQ DDATA NULLVAL EXTRN XDFLD PUNCHEDCARD
%token STAR_EXPR NUM ID

%type<str> STAR_EXPR NUM ID XDFLD PUNCHEDCARD
%type<oper> OPS TERM ARG LIST_OP OP
%type<expr> NAME SEGMENT CONST SRCH SUBSEQ DDATA NULLVAL EXTRN 
%type<args> ARGS
...
|	SEGMENT'='TERM		{ $$ = new ParserExpression("SEGMENT", NULL , $3 , true); }
|	CONST'='TERM		{ $$ = new ParserExpression("CONST", NULL , $3 , true); }
|	SRCH'='TERM		{ $$ = new ParserExpression("SRCH", NULL , $3 , true); }
|	SUBSEQ'='TERM		{ $$ = new ParserExpression("SUBSEQ", NULL , $3 , true); }
|	DDATA'='TERM		{ $$ = new ParserExpression("DDATA", NULL , $3 , true); }
|	NULLVAL'='TERM		{ $$ = new ParserExpression("NULLVAL", NULL , $3 , true); }
|	EXTRN'='TERM		{ $$ = new ParserExpression("EXTRN", NULL , $3 , true); }
;
